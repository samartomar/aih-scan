...
| --- | --- | --- | --- | --- | --- |
| Scan attestation v2 envelope | `src/observation/scan-attestation-v2.ts:225` | `predicateType: "https://aih.dev/ScanAttestationV2"`, `payloadType: "application/vnd.in-toto+json"`, `_type: "https://in-toto.io/Statement/v1"` | envelope 8 MiB | `TypeError("invalid ScanAttestationV2: …")` from `parseScanAttestationEnvelopeV2Json` | none |
| Scan attestation v2 predicate | `src/observation/scan-attestation-v2.ts:228` | `protocol: "ScanAttestationV2"` | as above | same throw | none |
| Scan candidate v2 | `src/observation/scan-attestation-v2.ts:132` | `protocol: "ScanCandidateV2"` | canonical candidate 2 MiB | same throw | none |
| Core contract identity inside the candidate | `src/observation/scan-attestation-v2.ts:138` | `coreContract.commit` and `coreContract.decisionSchemaSha256`, together one accepted pair | two strings | `TypeError` naming the invalid field; a candidate declaring a Core contract outside the accepted sets is refused, and so is an accepted commit declared with another accepted commit's digest (path `coreContract`) | the same accepted pairs in `src/core/core-contract-lock-v2.ts:19` |
...
| Core contract lock v2 — accepted commits | `src/core/core-contract-lock-v2.ts:31` | `AI_HARNESS_STRICT_V2_COMMIT_ACCEPTED`, newest last; the newest is the default emitted `AI_HARNESS_STRICT_V2_COMMIT` | `"unexpected Core commit"` for a commit outside the set |
| Core contract lock v2 — accepted decision-schema digests | `src/core/core-contract-lock-v2.ts:34` | `AI_HARNESS_DECISION_V2_SCHEMA_SHA256_ACCEPTED`, newest last; the newest is the default emitted `AI_HARNESS_DECISION_V2_SCHEMA_SHA256` | `"schema digest mismatch"` for a digest outside the set, and for bytes that do not hash to the declared digest |
| Core organization-evidence envelope schema | `src/core/core-contract-lock-v2.ts:43` | one pinned digest, `88c0a36e9177201660e773351958d89059c7d5b54e1c437d0afd06f48c5288bc`; the schema is byte-identical at every accepted Core commit | `"unexpected organization evidence schema digest"` |
