 ...
 *   The pinned 0.6.x analyzer prints a ScanResponse (`{scan_path_responses: [...]}`)
 *   instead; it is held to the same rules (every response names the root by an existing
 *   path, no failure ScanError anywhere, a non-failure note anywhere means not analyzed,
 *   every record and risk validated) and every present risk becomes one result. The 0.5.17
 *   shapes stay because the recorded parity outputs replayed against Core use them;
 ...
 * is no analysis; every record and risk is validated, none skipped.
...
function scanResponseResults(
  ...
  for (const response of responses) {
    ...
    // S2f: a non-failure note on a response means that path was not analyzed, whatever else
    // it carries.
    if (error === "note") throw new TypeError(NO_ANALYSIS);
  }
  return results;
